...
export default function WaveformVisualizer({ fileName, segments, audioUrl }: WaveformVisualizerProps) {
  ...
  const speakers = Object.keys(segmentsBySpeaker)
  ...
  // Generate mock waveform data with slight variation per speaker
  const generateWaveformData = (duration: number, speakerIndex: number = 0) => {
    const samples = 1000
    const data: number[] = []
  ...
  }, [waveformDatas, segments, currentTime, hoveredSegment, duration, currentSegment, speakers]);
  ...
  }, [segmentsBySpeaker, drawSpeakerWaveform, waveformDatas, segments]);
  ...
  useEffect(() => {
    ...
    } else if (duration === 0 && audioUrl) {
      // Set a default duration if we have an audio URL but no segments
      setDuration(60) // Default to 60 seconds
      const newWaveformDatas: Record<string, number[]> = {}
      speakers.forEach((speaker, index) => {
        newWaveformDatas[speaker] = generateWaveformData(60, index)
      })
      setWaveformDatas(newWaveformDatas)
    }
  }, [segments, audioUrl, duration, speakers])
  ...
  }, [drawSpeakerWaveform, segmentsBySpeaker, speakers])
  ...
  }, [isPlaying, currentSegment, updatePlayhead, drawWaveforms]);
  ...
  useEffect(() => {
    ...
    drawWaveforms();
  }, [hoveredSegment, currentSegment, drawWaveforms]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-2 border-b border-gray-200 dark:border-gray-700">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
          ...
          <Button onClick={playOriginal} variant="outline" size="sm" className="text-xs md:text-sm py-1 md:py-2 h-auto">
            ...
            <span className="ml-1 md:ml-2">Play Original</span>
          </Button>
        </div>
      </div>
