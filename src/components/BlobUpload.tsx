...
export default function BlobUpload({ onUploadComplete, onUploadError }: BlobUploadProps) {
    ...
    try {
      for (const audioFile of audioFiles) {
        ...
        try {
          ...
          const blob = await upload(audioFile.name, audioFile.file, {
            access: 'public',
            ...
            onUploadProgress: (progress) => {
              setAudioFiles(prev => 
                prev.map(af => 
                  af.id === audioFile.id 
                    ? { ...af, progress: progress.percentage }
                    : af
                )
              )
            },
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription className="text-sm md:text-base">
          Select WAV, MP3, FLAC, M4A, or AAC files for processing (up to 5TB per file)
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        >
          ...
          <p className="text-xs md:text-sm text-muted-foreground">
            Supports WAV, MP3, FLAC, M4A, AAC (up to 5TB per file)
          </p>
          <input
            ref={fileInputRef}
            type="file"
